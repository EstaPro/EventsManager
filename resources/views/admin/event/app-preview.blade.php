<div class="card mb-4 bg-gradient" style="background: linear-gradient(135deg, {{ $preview['primary_color'] ?? '#D4AF37' }} 0%, {{ $preview['secondary_color'] ?? '#0F172A' }} 100%);">
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                ...
                <div class="d-flex align-items-center gap-3 mb-3">
                    @if($preview['logo'])
                        <img src="{{ $preview['logo'] }}" alt="Logo" class="rounded" style="width: 60px; height: 60px; object-fit: contain; background: white; padding: 8px;">
                    @else
                    ...
                    @endif
                    <div>
                        <h5 class="mb-0">{{ $preview['event_name'] }}</h5>
                        <small class="opacity-75">Your event name will appear here</small>
                    </div>
                </div>
                <div class="d-flex gap-2 flex-wrap">
                    <div class="badge" style="background-color: {{ $preview['primary_color'] }}; color: white;">
                        Primary Color
                    </div>
                    <div class="badge" style="background-color: {{ $preview['secondary_color'] }}; color: white;">
                        Secondary Color
                    </div>
                </div>
            </div>
            <div class="col-md-4 text-center">
                <div class="phone-mockup mx-auto" style="width: 200px; height: 350px; background: white; border-radius: 30px; padding: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
                    <div class="screen" style="background: {{ $preview['secondary_color'] }}; height: 100%; border-radius: 20px; padding: 15px; color: white;">
                        ...
                        <div class="app-header mb-3 text-center">
                            @if($preview['logo'])
                                <img src="{{ $preview['logo'] }}" alt="Logo" style="width: 40px; height: 40px; margin-bottom: 10px;">
                            @endif
                            <div class="small fw-bold">{{ Str::limit($preview['event_name'], 20) }}</div>
                        </div>
                        <div class="menu-items">
                            <div class="p-2 rounded mb-2" style="background: {{ $preview['primary_color'] }};">
